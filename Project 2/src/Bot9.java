...
public class Bot9 {
    ...
    public static HashMap<CoordinateSet, Double> init_prob_matrix(Ship ship){
        ArrayList<Coordinate> path_list = ship.getPathList();
        HashMap<CoordinateSet, Double> prob_matrix = new HashMap<>();
        for(int i = 0; i < path_list.size(); i++){
            for(int j = i; j < path_list.size(); j++){
                prob_matrix.put(new CoordinateSet(path_list.get(i), path_list.get(j)), 0.0);
            }
        }

        int matrix_length = prob_matrix.size();
        for(Map.Entry<CoordinateSet, Double> set : prob_matrix.entrySet()){
            set.setValue((1.0)/matrix_length);
        }
        return prob_matrix;
    }

    // update probability table when bot entered a new cell
    public static void bot_enters_cell_probability_update(Ship ship, double[][] prob_table, HashMap<CoordinateSet, Double> prob_matrix){
        HashMap<CoordinateSet, Double> remove_list = new HashMap<>();
        // set the current bot location to probability 0.0 because it is not a leak
        for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
            if(entry.getKey().getCoor1().compare_coor(ship.getBotCoor()) || entry.getKey().getCoor2().compare_coor(ship.getBotCoor())){
                entry.setValue(0.0);
                remove_list.put(entry.getKey(), 0.0);
            }
        }

        // total probability after (100%) after current bot location probability drop down to 0%
        Double prob_leak_in_cell_i_and_prob_leak_in_cell_j_AND_leak_not_in_bot_loc = 0.0;

        // sum all the probability (the new 100% because current bot location probability drop down to 0%)
        for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
            Double prob_leak_in_i_and_prob_leak_in_j = entry.getValue();
            prob_leak_in_cell_i_and_prob_leak_in_cell_j_AND_leak_not_in_bot_loc += prob_leak_in_i_and_prob_leak_in_j;
        }
        ...
        for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
            entry.setValue((entry.getValue() / (prob_leak_in_cell_i_and_prob_leak_in_cell_j_AND_leak_not_in_bot_loc)));
        }
        ...
        //             for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
        //                 if(entry.getKey().getCoor1().compare_coor(curr) || entry.getKey().getCoor2().compare_coor(curr)){
        //                     sum += entry.getValue();
        //                 }
        //             }
        //             prob_table[i][j] = sum;
        ...
        for(Map.Entry<String, Coordinate> coor : ship.getHashPath().entrySet()){
            double sum = 0.0;
            for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
                if(entry.getKey().getCoor1().compare_coor(coor.getValue()) || entry.getKey().getCoor2().compare_coor(coor.getValue())){
                    sum += entry.getValue();
                }
            }
            prob_table[coor.getValue().getRow()][coor.getValue().getCol()] = sum;
        }

        for(Map.Entry<CoordinateSet, Double> entry : remove_list.entrySet()){
            prob_matrix.remove(entry.getKey());
        }
    }
    ...
    public static double beep_in_i(Ship ship, int[][] distance_table, HashMap<CoordinateSet, Double> prob_matrix){
        double prob_beep_in_i = 0.0;
        for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
            Double value = entry.getValue();

            double exp_d_k_i = Math.exp( (-1) * ship.getAlpha() * (distance_table[entry.getKey().getCoor1().getRow()][entry.getKey().getCoor1().getCol()] - 1) );
            double exp_d_k_j = Math.exp( (-1) * ship.getAlpha() * (distance_table[entry.getKey().getCoor2().getRow()][entry.getKey().getCoor2().getCol()] - 1) );
                
            prob_beep_in_i += (value * (exp_d_k_i + exp_d_k_j - (exp_d_k_i * exp_d_k_j)));
        }
        return prob_beep_in_i;
    }

    // update probability table after heard a beep
    public static void with_beep_probability_update(Ship ship, double[][] prob_table, int[][] distance_table, HashMap<CoordinateSet, Double> prob_matrix){
        double beep_in_i = beep_in_i(ship, distance_table, prob_matrix);
        ...
        for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
            ...
            double prob = ((exp_d_k_i + exp_d_k_j - (exp_d_k_i * exp_d_k_j)) * value) / beep_in_i;
    ...
    public static void without_beep_probability_update(Ship ship, double[][] prob_table, int[][] distance_table, HashMap<CoordinateSet, Double> prob_matrix){
        double beep_in_i = beep_in_i(ship, distance_table, prob_matrix);
       ...
       for(Map.Entry<CoordinateSet, Double> entry : prob_matrix.entrySet()){
            ...
            double prob = ((1.0 - (exp_d_k_i + exp_d_k_j - (exp_d_k_i * exp_d_k_j))) * value) / (1.0 - beep_in_i);
    ...
    public static int run_bot9(Ship ship){
        // get probablity of containing leak for each cell
        double[][] prob_table = init_leak_prob_table(ship);

        // get probability matrix for every possible pair of open cells.
        HashMap<CoordinateSet, Double> prob_matrix = init_prob_matrix(ship);
        ...
        while (!(visited_leak[0] == true && visited_leak[1] == true)) {
            ...
            bot_enters_cell_probability_update(ship, prob_table, prob_matrix);
            ...
            if(heard_beep == true){
                with_beep_probability_update(ship, prob_table, distance_table, prob_matrix);
            }else{ // heard_beep == false
                without_beep_probability_update(ship, prob_table, distance_table, prob_matrix);

                // maybe stay in the current position
                if(Math.random() <= Math.exp((-1)*ship.getAlpha()*(action_num))){
                    continue;
                }
            }
    ...
    public static int run_bot9_demo(Ship ship){
        // get probablity of containing leak for each cell
        double[][] prob_table = init_leak_prob_table(ship);
